...
function imageType(value: string | null) {
  ...
  if (!IMAGE_TYPES[mime]) throw new Error("仅支持 PNG、JPEG、GIF、WebP、AVIF、BMP、SVG 和 ICO 图片");
  return mime;
}
...
async function readImageSource(value: unknown) {
  ...
  if (typeof source.dataUrl === "string") {
    ...
    return { mime: imageType(match[1]), buffer: checkedBuffer(Buffer.from(match[2], "base64")) };
  }
  if (typeof source.url === "string") {
    ...
    const mime = imageType(response.headers.get("content-type"));
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        if (req.method === "POST" && url.pathname === ASSET_API) {
          try {
            ...
            const fileName = `${createHash("sha256").update(buffer).digest("hex").slice(0, 24)}.${IMAGE_TYPES[mime]}`;
        ...
        if (req.method === "GET" && url.pathname.startsWith(`${ASSET_API}/`)) {
          try {
            ...
            const match = /^([a-f0-9]{24})\.(png|jpg|gif|webp|avif|bmp|svg|ico)$/.exec(fileName ?? "");
            if (!match || parts.length !== 2) throw new Error("图片路径不合法");
            const extension = match[2];
            const mime = Object.entries(IMAGE_TYPES).find(([, value]) => value === extension)?.[0] ?? "application/octet-stream";
