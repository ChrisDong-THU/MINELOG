...
test("keeps navigation, rendering and local content storage in focused modules", async () => {
  ...
  assert.match(readerStyles, /\.markdown-body h3 \{[^}]*border-bottom: 1px solid #ddd8cd;[^}]*font-size: 19px;/s);
  assert.match(readerStyles, /\.markdown-body h4 \{[^}]*font-size: 16px;[^}]*font-weight: 700;/s);
  assert.doesNotMatch(readerStyles, /\.markdown-body h[134-6]::(?:before|after)/);
  assert.doesNotMatch(readerStyles, /\.markdown-body h[56] \{/);
