...
test("keeps navigation, rendering and local content storage in focused modules", async () => {
  ...
  assert.match(layout, /rel="preload" as="image" href="\/minecraft\/ui\/minelog-title\.png"/);
  assert.match(page, /minelog-title\.png[^>]*loading="eager"[^>]*decoding="sync"[^>]*fetchPriority="high"/);
  assert.match(page, /width=\{2048\} height=\{299\}/);
