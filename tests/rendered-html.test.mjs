...
test("keeps navigation, rendering and local content storage in focused modules", async () => {
  ...
  assert.match(readerStyles, /\.markdown-body h1 \{[^}]*font-size: 29px;[^}]*text-align: center;/s);
  assert.match(readerStyles, /\.markdown-body h1::after \{[^}]*left: 50%;[^}]*linear-gradient\(90deg, transparent[^}]*translateX\(-50%\)/s);
  ...
  assert.match(readerStyles, /\.markdown-body h3::before \{[^}]*width: 2px;[^}]*box-shadow:/s);
  assert.match(readerStyles, /\.markdown-body h3::after \{[^}]*width: 110px;[^}]*linear-gradient\(90deg,/s);
  assert.match(readerStyles, /\.markdown-body h4::before \{[^}]*width: 2px;[^}]*background: rgba\(125, 149, 86, \.52\)/s);
  assert.match(readerStyles, /\.markdown-body h5 \{[^}]*font-size: 15px;/s);
  assert.match(readerStyles, /\.markdown-body h6 \{[^}]*font-size: 13\.5px;/s);
  assert.doesNotMatch(readerStyles, /\.markdown-body h[4-6]::after/);
  assert.doesNotMatch(readerStyles, /\.markdown-body h[56]::before/);
  ...
  assert.match(page, /function ContentLoadingState/);
  assert.doesNotMatch(page, /content-loading-indicator/);
  assert.match(page, /Array\.from\(children\)\.map/);
