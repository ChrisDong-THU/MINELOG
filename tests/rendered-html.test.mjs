...
test("keeps navigation, rendering and local content storage in focused modules", async () => {
  ...
  const loadingState = await source("app/components/content-loading-state.tsx");
  ...
  assert.match(globalStyles, /\.game-hud:not\(\.immersive-hud-hidden\):not\(\.immersive-hud-awake\)[^}]*view-transition-name: page-hud/s);
  assert.match(globalStyles, /::view-transition-group\(page-hud\)[^}]*z-index: 100/s);
  assert.match(globalStyles, /\.game-hud \{[^}]*z-index: 70/s);
  assert.match(page, /className=\{`article-back-to-top\$\{showBackToTop \? " is-visible" : ""\}`\}/);
  assert.match(page, /aria-label="回到顶部"/);
  assert.match(page, /remaining <= Math\.max\(120, viewport\.clientHeight \* 0\.18\)/);
  assert.match(globalStyles, /\.article-back-to-top \{[^}]*z-index: 65/s);
  assert.match(homeStyles, /\.page-bottom-fade \{[^}]*z-index: 60[^}]*view-transition-name: page-bottom-fade/s);
  assert.match(globalStyles, /::view-transition-group\(page-bottom-fade\)[^}]*z-index: 90/s);
  assert.doesNotMatch(readerStyles, /\n\.game-hud \{[^}]*transition:/s);
  assert.match(readerStyles, /\.game-hud\.immersive-hud-hidden,[\s\S]*\.game-hud\.immersive-hud-awake \{[^}]*transition:/s);
  assert.match(page, /const autoHideHud = immersive/);
  assert.match(page, /className=\{`game-hud\$\{autoHideHud/);
  assert.match(page, /\{autoHideHud && <button className=\{`hud-wake-zone/);
  assert.match(page, /\{!immersive && <div className="page-bottom-fade"/);
  ...
  assert.match(readerStyles, /\.markdown-body h1,[\s\S]*\.markdown-body h6 \{[^}]*font-weight: 400;[^}]*text-wrap: balance;/s);
  assert.doesNotMatch(readerStyles, /\.markdown-body :is\(h1, h2, h3, h4, h5, h6\) strong/);
  assert.match(readerStyles, /\.markdown-body h1 \{[^}]*font-size: clamp\(27px, 3\.4vw, 30px\);[^}]*text-align: center;/s);
  assert.match(readerStyles, /\.markdown-body h1::after \{[^}]*width: 58px;[^}]*linear-gradient\(90deg,/s);
  assert.doesNotMatch(readerStyles, /\.markdown-body h2::before/);
  assert.match(readerStyles, /\.markdown-body h2::after \{[^}]*right: 0;[^}]*linear-gradient\(90deg,/s);
  assert.match(readerStyles, /\.markdown-body h3 \{[^}]*font-size: 19px;/s);
  assert.match(readerStyles, /\.markdown-body h4 \{[^}]*font-size: 17px;/s);
  assert.match(readerStyles, /\.markdown-body h5 \{[^}]*font-size: 15\.5px;/s);
  assert.match(readerStyles, /\.markdown-body h6 \{[^}]*font-size: 14px;/s);
  assert.doesNotMatch(readerStyles, /\.markdown-body h[3-6]::(?:before|after)/);
  assert.match(readerStyles, /\.markdown-body \.katex-display::-webkit-scrollbar \{ height: 4px; \}/);
  assert.match(readerStyles, /\.markdown-body \.katex-display::-webkit-scrollbar-track \{ background: transparent; \}/);
  ...
  assert.match(readerStyles, /\.reader-details \{[^}]*grid-template-columns: minmax\(0, 29fr\) minmax\(0, 42fr\) minmax\(0, 29fr\)/s);
  assert.match(readerStyles, /\.reader-details \{[^}]*margin: 0 8px/s);
  assert.match(readerStyles, /\.reader-details div \{[^}]*padding: 0 4px[^}]*text-align: center/s);
  assert.match(readerStyles, /\.reader-detail-updated dd \{[^}]*font-variant-numeric: tabular-nums[^}]*text-overflow: clip/s);
  assert.match(articleReader, /className="reader-detail-updated"><dt>更新<\/dt><dd>2026\.\{article\.date\}<\/dd>/);
  assert.doesNotMatch(readerStyles, /\.reader-detail-author dd/);
  assert.match(articleReader, /<dt>浏览<\/dt><dd>\{views === null \? "—" : formatArticleViews\(views\)\}<\/dd>/);
  ...
  assert.match(page, /\.finally\(\(\) => markdownRequests\.current\.delete\(key\)\)/);
  assert.match(page, /import \{ ContentLoadingState \} from "\.\/components\/content-loading-state"/);
  assert.match(loadingState, /export function ContentLoadingState/);
  assert.doesNotMatch(page, /content-loading-indicator/);
  assert.match(loadingState, /Array\.from\(children\)\.map/);
  assert.match(editor, /<ContentLoadingState>\{waitingMessage\}<\/ContentLoadingState>/);
  assert.match(globalStyles, /@keyframes content-loading-character/);
  assert.match(globalStyles, /@media \(prefers-reduced-motion: reduce\)[\s\S]*\.content-loading-label > span/s);
  ...
  assert.match(editor, /setWaitingMessage\("正在保存文章正文…"\)/);
  assert.match(editor, /setWaitingMessage\("正在删除文章…"\)/);
